...
# Ensure default values (BOOT_B_LEFT=3 instead of 0)
ensure_env_defaults() {
    local order=$(uboot_env_get BOOT_ORDER)
    if [ -z "$order" ]; then
        fw_setenv BOOT_ORDER "B A" 2>/dev/null || true  # Prefer B by default
...
case "$1" in
    get-primary)
        ensure_env_defaults
        # RAUC contract: get-primary must return the NEXT slot (from BOOT_ORDER)
        BOOT_ORDER=$(uboot_env_get BOOT_ORDER)
        if [ -n "$BOOT_ORDER" ]; then
            echo "$BOOT_ORDER" | awk '{print $1}'
        else
            echo "A"  # reasonable default
        fi
        ;;
